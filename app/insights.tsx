...
import { Dimensions, ScrollView, StyleSheet, Text, View, useColorScheme, TouchableOpacity } from "react-native";
...
export default function InsightsScreen() {
    const { profile, sessions, moodHistory, setCurrentScreen } = useAppContext();
    ...
    const calculateDominantMood = () => {
        ...
        const topMoodKey = Object.keys(moodCounts).reduce((a, b) =>
            ...
            , Object.keys(moodCounts)[0] || 'neutral');
    ...
    // 4. Generate Daily Data for Chart
    const getWeeklyData = () => {
        const data = [...sessions].slice(0, 7).reverse();
        while (data.length < 7) {
            data.unshift({ mood: 'neutral' } as any);
        }
    ...
    return (
        <View style={[styles.mainContainer, darkMode ? { backgroundColor: "#0B1220" } : { backgroundColor: "#F3F4F6" }]}>
            {/* Custom Header (Green) */}
            <View style={[styles.headerGreen, darkMode ? { backgroundColor: "#0B1220" } : { backgroundColor: "#065F46" }]}>
                <TouchableOpacity onPress={() => setCurrentScreen("dashboard")} style={styles.backButton}>
                    <Feather name="arrow-left" size={24} color="white" />
                </TouchableOpacity>

                <View style={{ flex: 1, alignItems: 'center' }}>
                    <Text style={styles.headerTitleGreen}>{t("insights_title")}</Text>
                    <Text style={styles.headerSubtitleGreen}>{t("insights_subtitle")}</Text>
                </View>
...
const styles = StyleSheet.create({
    ...
    headerTitleGreen: { color: "white", fontSize: 18, fontWeight: "800" },
    headerSubtitleGreen: { color: "rgba(255,255,255,0.8)", fontSize: 12, fontWeight: "500" },
    ...
    scoreLabel: { fontSize: 14, fontWeight: "600", marginBottom: 4, textTransform: 'uppercase', letterSpacing: 0.5 },
    scoreBig: { fontSize: 42, fontWeight: "900", lineHeight: 48 },
    scoreTotal: { fontSize: 18, fontWeight: "600", marginLeft: 4 },
    ...
    barTrack: { height: "100%", width: 8, backgroundColor: "rgba(0,0,0,0.05)", borderRadius: 4, justifyContent: 'flex-end', overflow: 'hidden' },
    barFill: { width: "100%", borderRadius: 4 },
    barLabel: { marginTop: 8, fontSize: 11, fontWeight: "500" },
    grid: { flexDirection: 'row', gap: 12, marginBottom: 16 },
    statCard: { flex: 1, borderRadius: 20, padding: 16, flexDirection: 'row', alignItems: 'center', gap: 12, elevation: 2 },
    statIcon: { width: 40, height: 40, borderRadius: 12, alignItems: 'center', justifyContent: 'center' },
    statValue: { fontSize: 18, fontWeight: "800" },
    statLabel: { fontSize: 12, color: "#64748B" },
    adviceCard: { backgroundColor: "#2563EB", borderRadius: 24, padding: 24, marginBottom: 16 },
    adviceHeader: { flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 12, opacity: 0.8 },
    adviceTitleSmall: { color: "white", fontSize: 14, fontWeight: "700", textTransform: 'uppercase' },
    adviceContent: { color: "white", fontSize: 18, fontWeight: "600", lineHeight: 26 },
});
